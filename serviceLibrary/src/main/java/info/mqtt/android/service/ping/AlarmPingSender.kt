...
internal class AlarmPingSender(val service: MqttService) : MqttPingSender {
    ...
    override fun start() {
        ...
        Log.d("###","Register AlarmReceiver to MqttService$action")
    ...
    override fun stop() {
        Log.d("###","Unregister AlarmReceiver to MqttService ${clientComms!!.client.clientId}")
        if (hasStarted) {
    ...
    override fun schedule(delayInMilliseconds: Long) {
        ...
        Log.d("###","Schedule next alarm at $nextAlarmInMilliseconds ms")
        val alarmManager = service.getSystemService(Service.ALARM_SERVICE) as AlarmManager
        if (Build.VERSION.SDK_INT >= 23) {
            // In SDK 23 and above, dosing will prevent setExact, setExactAndAllowWhileIdle will force
            // the device to run this task whilst dosing.
            Log.d("###","Alarm schedule using setExactAndAllowWhileIdle, next: $delayInMilliseconds")
            alarmManager.setExactAndAllowWhileIdle(AlarmManager.ELAPSED_REALTIME_WAKEUP, nextAlarmInMilliseconds, pendingIntent)
        } else
            Log.d("###","Alarm schedule using setExact, delay: $delayInMilliseconds")
    ...
    fun backgroundExecute(comms: ClientComms?): Boolean {
        ...
        try {
            if (token != null) {
                ...
                Log.d("###","Ping background : Ping command was not sent by the client.")
            }
        } catch (e: MqttException) {
            Log.d("###","Ping background : Ignore MQTT exception : ${e.message}")
        } catch (ex: Exception) {
            Log.d("###","Ping background : Ignore unknown exception : ${ex.message}")
        }
        return success
    }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                measureTimeMillis {
                    ...
                    Log.d("###","Request done $response")

                    if (wakelock.isHeld) {
                        wakelock.release()
                    }
                }.also {
                    Log.d("###","Completed in $it ms")
                }
            }
        }
    }
