...
/**
 * Define the MovementState ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */
...
public class MovementState extends Sequence {
    ...
    public MovementEventList state_time_speed;
	...
	/**
	 * Define the _seq35 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_MovementState;
	...
	__state_time_speed("state-time-speed"),
	...
	// Encode field 'state-time-speed'
	try {
	    MovementEventList item1 = data.state_time_speed;

	    nbits += MovementEventList.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("state-time-speed", "MovementEventList");
	    throw ee;
	}
	...
	// Decode field 'state-time-speed'
	try {
	    if (data.state_time_speed == null)
		data.state_time_speed = new MovementEventList();
	    data.state_time_speed.decodeValue(coder, source, data.state_time_speed);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("state-time-speed", "MovementEventList");
	    throw de;
	}
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'state-time-speed'
	try {
	    MovementEventList item1 = this.state_time_speed;

	    {
		sink.writeSeparator();
		sink.encodeKey("state-time-speed");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
		    ...
		    break;
		    case __state_time_speed:
		    // Decode field 'state-time-speed'
		    try {
			if (present0[2])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.state_time_speed == null)
			    this.state_time_speed = new MovementEventList();
			this.state_time_speed.decodeValue(coder, source);
			present0[2] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("state-time-speed", "MovementEventList");
	...
	if (!present0[2])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'state-time-speed'");
	if (!present0[3])
	...
	try {
	    writer.print(',');
	    printer.newLine(writer);
	    writer.print("state-time-speed ");
	    this.state_time_speed.printValue(printer, writer);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.maneuverAssistList != null) {
	...
	if (this.movementName != null) {
	    if (that.movementName == null || !this.movementName.equalTo(that.movementName))
		return false;
	} else if (that.movementName != null)
	    return false;
	if (!this.signalGroup.equalTo(that.signalGroup))
	    return false;
	if (!this.state_time_speed.equalTo(that.state_time_speed))
	    return false;
	if (this.maneuverAssistList != null) {
	...
	copy.state_time_speed = state_time_speed.clone();
	...
	hash = 41 * hash + (this.state_time_speed != null ? this.state_time_speed.hashCode() : 0);
