...
/**
 * Define the TravelerInformation ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class TravelerInformation extends Sequence implements JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
    public MsgCount msgCnt;
    public MinuteOfTheYear timeStamp;
    public UniqueMSGID packetID;
    public URL_Base urlB;
    public TravelerDataFrameList dataFrames;
	...
	/**
	 * Define the _seq19 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_TravelerInformation;
	...
	try {
	    TravelerDataFrameList item1 = data.dataFrames;

	    nbits += TravelerDataFrameList.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("dataFrames", "TravelerDataFrameList");
	    throw ee;
	}
	...
	try {
		...
		data.dataFrames = new TravelerDataFrameList();
	    data.dataFrames.decodeValue(coder, source, data.dataFrames);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("dataFrames", "TravelerDataFrameList");
	    throw de;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'dataFrames'
	try {
	    TravelerDataFrameList item1 = this.dataFrames;
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (this.dataFrames == null)
			    this.dataFrames = new TravelerDataFrameList();
	...
	if (!present0[3])
	    this.urlB = null;
	if (!present0[4])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'dataFrames'");
	if (!present0[5])
	    this.regional = null;
	return this;
    }
	...
	try {
	    ...
	    this.dataFrames.printValue(printer, writer);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.regional != null) {
	...
	if (this.timeStamp != null) {
	    ...
	    return false;
	if (this.packetID != null) {
	    if (that.packetID == null || !this.packetID.equalTo(that.packetID))
		return false;
	} else if (that.packetID != null)
	    return false;
	if (this.urlB != null) {
	    if (that.urlB == null || !this.urlB.equalTo(that.urlB))
		return false;
	} else if (that.urlB != null)
	    return false;
