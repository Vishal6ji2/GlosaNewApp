...
/**
 * Define the TireDataList ASN.1 type included in the DSRC ASN.1 module.
 * @see SequenceOf
 */
...
public class TireDataList extends SequenceOf<TireData> {
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, TireDataList data)
	...
	while (total_len0 > 0) {
	    try {
		TireData item1 = data.elements.get(idx0);

		nbits += TireData.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendElementContext(null, "TireData", idx0);
		throw ee;
	    }
     ...
     * @exclude
     */
    public static TireDataList decodeValue(PerCoder coder, InputBitStream source, TireDataList data)
	...
	if (data.elements != null)
	    ...
	    data.elements = new java.util.ArrayList<TireData>(fragmentLength0);
	while (idx0 > 0) {
	    try {
		TireData item1 = new TireData();

		data.elements.add(item1);
		item1.decodeValue(coder, source, item1);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendElementContext(null, "TireData", total_len0);
		throw de;
	    }
	    --idx0; ++total_len0;
	    if (idx0 == 0) {
	...
	if (total_len0 > 0) {
	    while (true) {
		try {
		    TireData item1 = this.elements.get(idx0);
     ...
     * @exclude
     */
    public TireDataList decodeValue(JsonCoder coder, JsonReader source)
	...
	if (this.elements != null)
	    ...
	    this.elements = new java.util.ArrayList<TireData>(total_len0);
	...
	if (coder.hasMoreElements(source, true))
		...
		try {
		    ...
		    DecoderException de = DecoderException.wrapException(e);
		    de.appendElementContext(null, "TireData", idx0);
		    throw de;
		}
		++idx0;
    ...
    public void printValue(DataPrinter printer, java.io.PrintWriter writer)
	    throws Exception
    {
	java.util.ArrayList<TireData> temp0 = this.elements;
    ...
    {
	return equalTo((TireDataList)that);
    }
    ...
    public boolean equalTo(TireDataList that) {
     ...
     * Clone 'this' object.
     */
    public TireDataList clone() {
	TireDataList copy = (TireDataList)super.clone();
	copy.elements = new java.util.ArrayList<TireData>(elements.size());
	for (TireData element : elements) {
