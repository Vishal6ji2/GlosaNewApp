...
/**
 * Define the SignalStatusMessage ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class SignalStatusMessage extends Sequence implements JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
    public MinuteOfTheYear timeStamp;
    public DSecond second;
    public MsgCount sequenceNumber;
    public SignalStatusList status;
	...
	/**
	 * Define the _seq18 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_SignalStatusMessage;
	...
	// Encode field 'status'
	try {
	    SignalStatusList item1 = data.status;

	    nbits += SignalStatusList.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("status", "SignalStatusList");
	    throw ee;
	}
	...
	try {
	    if (data.status == null)
		data.status = new SignalStatusList();
	    data.status.decodeValue(coder, source, data.status);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("status", "SignalStatusList");
	    throw de;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'status'
	try {
	    SignalStatusList item1 = this.status;
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (present0[3])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.status == null)
			    this.status = new SignalStatusList();
