...
/**
 * Define the NodeXY ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class NodeXY extends Sequence {
    public NodeOffsetPointXY delta;
    public NodeAttributeSetXY attributes;
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, NodeXY data)
	...
	// Encode field 'delta'
	try {
	    NodeOffsetPointXY item1 = data.delta;

	    nbits += NodeOffsetPointXY.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("delta", "NodeOffsetPointXY");
	    throw ee;
	}
	if (data.attributes != null) {
	    // Encode field 'attributes'
	    try {
		NodeAttributeSetXY item1 = data.attributes;

		nbits += NodeAttributeSetXY.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("attributes", "NodeAttributeSetXY");
		throw ee;
	    }
	}
     ...
     * @exclude
     */
    public static NodeXY decodeValue(PerCoder coder, InputBitStream source, NodeXY data)
	...
	try {
	    if (data.delta == null)
		data.delta = new NodeOffsetPointXY();
	    data.delta.decodeValue(coder, source, data.delta);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("delta", "NodeOffsetPointXY");
	    throw de;
	}
	if (has_attributes0) {
	    ...
	    try {
		if (data.attributes == null)
		    data.attributes = new NodeAttributeSetXY();
		data.attributes.decodeValue(coder, source, data.attributes);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("attributes", "NodeAttributeSetXY");
		throw de;
	    }
	} else {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'delta'
	try {
	    NodeOffsetPointXY item1 = this.delta;
	...
	// Encode field 'attributes'
	try {
	    NodeAttributeSetXY item1 = this.attributes;
     ...
     * @exclude
     */
    public NodeXY decodeValue(JsonCoder coder, JsonReader source)
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (this.delta == null)
			    this.delta = new NodeOffsetPointXY();
			...
			if (!coder.isNullValue(source)) {
				...
				this.attributes = new NodeAttributeSetXY();
    ...
    public boolean equalTo(NodeXY that) {
     ...
     */
    public NodeXY clone() {
	NodeXY copy = (NodeXY)super.clone();
