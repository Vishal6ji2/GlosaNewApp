...
/**
 * Define the SPAT ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class SPAT extends Sequence implements JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
	...
	/**
	 * Define the _seq15 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_SPAT;
	...
	try {
	    IntersectionStateList item1 = data.intersections;

	    nbits += IntersectionStateList.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("intersections", "IntersectionStateList");
	    throw ee;
	}
	...
	try {
		...
		data.intersections = new IntersectionStateList();
	    data.intersections.decodeValue(coder, source, data.intersections);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("intersections", "IntersectionStateList");
	    throw de;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'intersections'
	try {
	    IntersectionStateList item1 = this.intersections;
	...
	if (!present0[1])
	    this.name = null;
	if (!present0[2])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'intersections'");
	if (!present0[3])
	    this.regional = null;
	return this;
    }
