...
/**
 * Define the RoadSegment ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */
...
public class RoadSegment extends Sequence {
    ...
    public RoadLaneSetList roadLaneSet;
	...
	/**
	 * Define the _seq41 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_RoadSegment;
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, RoadSegment data)
	    throws IOException, EncoderException, EncodeFailedException
    {
	int nbits = 0;
	...
	if (data.name != null) {
		...
		nbits += com.oss.coders.per.PerKMCString.encode(coder, item1.stringValue(), 1, 63, RoadSegment._cEPAInfo_name, sink);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
	...
	// Encode field 'roadLaneSet'
	try {
	    RoadLaneSetList item1 = data.roadLaneSet;

	    nbits += RoadLaneSetList.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("roadLaneSet", "RoadLaneSetList");
	    throw ee;
	}
     ...
     * @exclude
     */
    public static RoadSegment decodeValue(PerCoder coder, InputBitStream source, RoadSegment data)
	    throws IOException, DecoderException, DecodeFailedException
    {
	...
	if (has_name0) {
	    // Decode field 'name'
	    try {
		data.name = new DescriptiveName(com.oss.coders.per.PerKMCString.decode(coder, source, 1, 63, RoadSegment._cEPAInfo_name));
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	// Decode field 'roadLaneSet'
	try {
	    if (data.roadLaneSet == null)
		data.roadLaneSet = new RoadLaneSetList();
	    data.roadLaneSet.decodeValue(coder, source, data.roadLaneSet);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("roadLaneSet", "RoadLaneSetList");
	    throw de;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'roadLaneSet'
	try {
	    RoadLaneSetList item1 = this.roadLaneSet;

	    {
		sink.writeSeparator();
		sink.encodeKey("roadLaneSet");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
     ...
     * @exclude
     */
    public RoadSegment decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (present0[6])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.roadLaneSet == null)
			    this.roadLaneSet = new RoadLaneSetList();
			this.roadLaneSet.decodeValue(coder, source);
			present0[6] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("roadLaneSet", "RoadLaneSetList");
	...
	if (!present0[6])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'roadLaneSet'");
	if (!present0[7])
	    this.regional = null;
	return this;
    }
	...
	try {
	    writer.print(',');
	    printer.newLine(writer);
	    writer.print("roadLaneSet ");
	    this.roadLaneSet.printValue(printer, writer);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.regional != null) {
    ...
    public boolean abstractEqualTo(AbstractData that)
    {
	return equalTo((RoadSegment)that);
    }
    ...
    public boolean equalTo(RoadSegment that) {
	if (this.name != null) {
	...
	if (this.speedLimits != null) {
	    ...
	    return false;
	if (!this.roadLaneSet.equalTo(that.roadLaneSet))
	    return false;
	if (this.regional != null) {
	...
	RoadSegment copy = (RoadSegment)super.clone();
	if (name != null) {
	...
	copy.roadLaneSet = roadLaneSet.clone();
	if (regional != null) {
	...
	hash = 41 * hash + (this.roadLaneSet != null ? this.roadLaneSet.hashCode() : 0);
