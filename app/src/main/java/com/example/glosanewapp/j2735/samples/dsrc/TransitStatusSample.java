...
import com.example.glosanewapp.j2735.dsrc.TransitStatus;
...
/**
 * Define sample code for the TransitStatus ASN.1 type included in the DSRC ASN.1 module.
 * @see TransitStatus
 */

public class TransitStatusSample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private TransitStatusSample() {}

    /**
     * Create Sample Value.
     */
    public static TransitStatus createSampleValue()
    {
	TransitStatus value = new TransitStatus(new byte[]
	{
	    (byte)0xC0
	},
	2);
    ...
    public static void printValue(TransitStatus value, PrintStream s)
    {
	
	s.print(value);
    }
    
    public static int encodeDecodeAndPrint(TransitStatus value, int run)
    {
	...
	TransitStatus decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (TransitStatus)coder.decode(source, value);
