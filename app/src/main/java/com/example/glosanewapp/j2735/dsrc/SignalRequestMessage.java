...
/**
 * Define the SignalRequestMessage ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class SignalRequestMessage extends Sequence implements JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
    public MinuteOfTheYear timeStamp;
    public DSecond second;
    public MsgCount sequenceNumber;
    public SignalRequestList requests;
    public RequestorDescription requestor;
	...
	/**
	 * Define the _seq17 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_SignalRequestMessage;
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, SignalRequestMessage data)
	    throws IOException, EncoderException, EncodeFailedException
    {
	int nbits = 0;
	...
	sink.writeBit(data.sequenceNumber != null);
	sink.writeBit(data.requests != null);
	...
	if (data.requests != null) {
	    ...
	    try {
		SignalRequestList item1 = data.requests;

		nbits += SignalRequestList.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("requests", "SignalRequestList");
		throw ee;
	    }
	}
	// Encode field 'requestor'
	try {
	    RequestorDescription item1 = data.requestor;

	    nbits += RequestorDescription.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("requestor", "RequestorDescription");
	    throw ee;
	}
	...
	try {
	    ...
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "SignalRequestMessage");
	    throw ee;
	}
    }
     ...
     * @exclude
     */
    public static SignalRequestMessage decodeValue(PerCoder coder, InputBitStream source, SignalRequestMessage data)
	    throws IOException, DecoderException, DecodeFailedException
    {
	...
	boolean has_sequenceNumber0 = source.readBit();
	boolean has_requests0 = source.readBit();
	...
	if (has_requests0) {
	    ...
	    try {
		if (data.requests == null)
		    data.requests = new SignalRequestList();
		data.requests.decodeValue(coder, source, data.requests);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("requests", "SignalRequestList");
		throw de;
	    }
	} else {
	    data.requests = null;
	}
	// Decode field 'requestor'
	try {
	    if (data.requestor == null)
		data.requestor = new RequestorDescription();
	    data.requestor.decodeValue(coder, source, data.requestor);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("requestor", "RequestorDescription");
	    throw de;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	try {
	    ...
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext(null, "SignalRequestMessage");
	    throw de;
	}
    }
	...
	try {
		...
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "sequenceNumber");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("sequenceNumber", "MsgCount");
	...
	// Encode field 'requests'
	try {
	    SignalRequestList item1 = this.requests;
		...
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "requests");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("requests", "SignalRequestList");
	...
	// Encode field 'requestor'
	try {
	    RequestorDescription item1 = this.requestor;

	    {
		sink.writeSeparator();
		sink.encodeKey("requestor");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
     ...
     * @exclude
     */
    public SignalRequestMessage decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (!coder.isNullValue(source)) {
				...
				this.requests = new SignalRequestList();
			...
			if (this.requestor == null)
			    this.requestor = new RequestorDescription();
			this.requestor.decodeValue(coder, source);
			present0[4] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("requestor", "RequestorDescription");
	...
	if (!present0[3])
	    this.requests = null;
	if (!present0[4])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'requestor'");
	if (!present0[5])
	    this.regional = null;
	return this;
    }
	...
	try {
	    writer.print(',');
	    printer.newLine(writer);
	    writer.print("requestor ");
	    this.requestor.printValue(printer, writer);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.regional != null) {
    ...
    public boolean abstractEqualTo(AbstractData that)
    {
	return equalTo((SignalRequestMessage)that);
    }
    ...
    public boolean equalTo(SignalRequestMessage that) {
	if (this.timeStamp != null) {
	...
	if (!this.second.equalTo(that.second))
	    return false;
	if (this.sequenceNumber != null) {
	    if (that.sequenceNumber == null || !this.sequenceNumber.equalTo(that.sequenceNumber))
		return false;
	} else if (that.sequenceNumber != null)
	    return false;
	if (this.requests != null) {
	    if (that.requests == null || !this.requests.equalTo(that.requests))
		return false;
	} else if (that.requests != null)
	    return false;
	if (!this.requestor.equalTo(that.requestor))
	    return false;
	if (this.regional != null) {
     ...
     * Clone 'this' object.
     */
    public SignalRequestMessage clone() {
	SignalRequestMessage copy = (SignalRequestMessage)super.clone();
	if (timeStamp != null) {
	...
	copy.requestor = requestor.clone();
	if (regional != null) {
	...
	hash = 41 * hash + (this.requestor != null ? this.requestor.hashCode() : 0);
