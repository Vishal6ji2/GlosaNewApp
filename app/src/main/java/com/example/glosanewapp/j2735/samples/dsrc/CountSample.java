...
import com.example.glosanewapp.j2735.dsrc.Count;
...
/**
 * Define sample code for the Count ASN.1 type included in the DSRC ASN.1 module.
 * @see Count
 */

public class CountSample extends SampleUtil {
     ...
     * Create Sample Value.
     */
    public static Count createSampleValue()
    {
	Count value = new Count(0);
	return value;
    }
    
    public static void printValue(Count value, PrintStream s)
    {
	
	s.print(value.longValue());
    }
    
    public static int encodeDecodeAndPrint(Count value, int run)
    {
	...
	try {
	    ...
	    decoded = (Count)coder.decode(source, value);
