...
/**
 * Define the RestrictionUserType ASN.1 type included in the DSRC ASN.1 module.
 * @see Choice
 */
...
public class RestrictionUserType extends Choice {
    ...
    /**
     * Define the _seqof2 ASN.1 type included in the DSRC ASN.1 module.
     * @see SequenceOf
     */
	...
	/**
	 * Define the _seq40 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_RestrictionUserType;
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, RestrictionUserType data)
	    throws IOException, EncoderException, EncodeFailedException
    {
	...
	switch (idx0) {
		...
		// Encode alternative 'basicType'
		try {
		    RestrictionAppliesTo item1 = (RestrictionAppliesTo)data.mChosenValue;
		    int idx1;
		    ...
		    EncoderException ee = EncoderException.wrapException(e);
		    ee.appendElementContext("basicType", "RestrictionAppliesTo", 0);
		    throw ee;
		}
		break;
     ...
     * @exclude
     */
    public static RestrictionUserType decodeValue(PerCoder coder, InputBitStream source, RestrictionUserType data)
	    throws IOException, DecoderException, DecodeFailedException
    {
	int idx0 = 0;
	...
	if (extroot0) {
	    ...
	    switch (idx0) {
		case basicType_chosen:
		    // Decode alternative 'basicType'
		    try {
			RestrictionAppliesTo item1;
			int idx1;
			RestrictionAppliesTo temp1;
			...
			if (extroot1) {
			    ...
			    temp1 = RestrictionAppliesTo.valueAt(idx1);
			} else {
			    idx1 = 14 + (int)coder.decodeNormallySmallNumber(source);
			    if (idx1 < 0)
				throw new DecoderException(ExceptionDescriptor._not_enumerated, null, "index = " + idx1);
			    temp1 = RestrictionAppliesTo.unknownEnumerator();
			}
			item1 = temp1;
			data.mChosenValue = item1;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendElementContext("basicType", "RestrictionAppliesTo", 0);
			throw de;
		    }
		    break;
		case regional_chosen:
	...
	case basicType_chosen:
	    // Encode alternative 'basicType'
	    try {
		RestrictionAppliesTo item1 = (RestrictionAppliesTo)this.mChosenValue;
     ...
     * @exclude
     */
    public RestrictionUserType decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	coder.decodeObject(source);
	String tag0 = coder.nextProperty(source);
	...
	switch (t_tag0) {
		...
		try {
		    ...
		    idx1 = coder.resolveName(RestrictionAppliesTo.cConstantNameList, content1);
		    if (idx1 < 0 )
			temp1 = RestrictionAppliesTo.unknownEnumerator();
		    else
			temp1 = RestrictionAppliesTo.cNamedNumbers[idx1];
		    item1 = temp1;
		    this.mChosenValue = item1;
		    this.mChosenFlag = basicType_chosen;
		} catch (Exception e) {
		    DecoderException de = DecoderException.wrapException(e);
		    de.appendElementContext("basicType", "RestrictionAppliesTo", 0);
		    throw de;
 ...
 // coder.setUnknownExtensionFound();
	...
	case basicType_chosen:
		...
		printer.print(((RestrictionAppliesTo)this.mChosenValue), writer, ((RestrictionAppliesTo)this.mChosenValue).cConstantNameList);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	    break;
     ...
     * Clone 'this' object.
     */
    public RestrictionUserType clone() {
	return (RestrictionUserType)super.clone();
    }
