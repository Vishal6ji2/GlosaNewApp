...
import com.example.glosanewapp.j2735.dsrc.DMonthDay;
...
/**
 * Define sample code for the DMonthDay ASN.1 type included in the DSRC ASN.1 module.
 * @see DMonthDay
 */

public class DMonthDaySample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private DMonthDaySample() {}

    /**
     * Create Sample Value.
     */
    public static DMonthDay createSampleValue()
    {
	DMonthDay value = new DMonthDay();
	value.setMonth(new DMonth(0));
	value.setDay(new DDay(0));
	return value;
    }
    
    public static void printValue(DMonthDay value, PrintStream s)
    {
	{
	    s.print("{");
	    ++indentlevel;
    ...
    public static int encodeDecodeAndPrint(DMonthDay value, int run)
    {
	...
	DMonthDay decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (DMonthDay)coder.decode(source, value);
