...
/**
 * Define the MovementEvent ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class MovementEvent extends Sequence {
    public MovementPhaseState eventState;
    public TimeChangeDetails timing;
    public AdvisorySpeedList speeds;
	...
	/**
	 * Define the _seq34 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_MovementEvent;
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, MovementEvent data)
	    throws IOException, EncoderException, EncodeFailedException
    {
	int nbits = 0;

	sink.writeBit(false);
	sink.writeBit(data.timing != null);
	sink.writeBit(data.speeds != null);
	sink.writeBit(data.regional != null);
	nbits += 4;
	// Encode field 'eventState'
	try {
	    MovementPhaseState item1 = data.eventState;
	    ...
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("eventState", "MovementPhaseState");
	    throw ee;
	}
	if (data.timing != null) {
	    // Encode field 'timing'
	    try {
		TimeChangeDetails item1 = data.timing;

		nbits += TimeChangeDetails.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("timing", "TimeChangeDetails");
		throw ee;
	    }
	}
	if (data.speeds != null) {
	    // Encode field 'speeds'
	    try {
		AdvisorySpeedList item1 = data.speeds;

		nbits += AdvisorySpeedList.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("speeds", "AdvisorySpeedList");
		throw ee;
	    }
	}
     ...
     * @exclude
     */
    public static MovementEvent decodeValue(PerCoder coder, InputBitStream source, MovementEvent data)
	    throws IOException, DecoderException, DecodeFailedException
    {
	...
	boolean has_timing0 = source.readBit();
	boolean has_speeds0 = source.readBit();
	boolean has_regional0 = source.readBit();
    /** Decode root fields **/
	// Decode field 'eventState'
	try {
	    int idx1;
	    ...
	    data.eventState = MovementPhaseState.valueAt(idx1);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("eventState", "MovementPhaseState");
	    throw de;
	}
	if (has_timing0) {
	    // Decode field 'timing'
	    try {
		if (data.timing == null)
		    data.timing = new TimeChangeDetails();
		data.timing.decodeValue(coder, source, data.timing);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("timing", "TimeChangeDetails");
		throw de;
	    }
	} else {
	    data.timing = null;
	...
	if (has_speeds0) {
	    ...
	    try {
		if (data.speeds == null)
		    data.speeds = new AdvisorySpeedList();
		data.speeds.decodeValue(coder, source, data.speeds);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("speeds", "AdvisorySpeedList");
		throw de;
	    }
	} else {
	    data.speeds = null;
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'eventState'
	try {
	    MovementPhaseState item1 = this.eventState;

	    {
		sink.encodeKey("eventState");
	...
	// Encode field 'timing'
	try {
	    TimeChangeDetails item1 = this.timing;

	    if (item1 != null) {
		{
		    sink.writeSeparator();
		    sink.encodeKey("timing");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "timing");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("timing", "TimeChangeDetails");
	...
	// Encode field 'speeds'
	try {
	    AdvisorySpeedList item1 = this.speeds;

	    if (item1 != null) {
		{
		    sink.writeSeparator();
		    sink.encodeKey("speeds");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "speeds");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("speeds", "AdvisorySpeedList");
     ...
     * @exclude
     */
    public MovementEvent decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			int idx1;
			MovementPhaseState temp1;

			if (present0[0])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			idx1 = coder.resolveName(MovementPhaseState.cConstantNameList, content1);
			if (idx1 < 0 )
			    throw new DecoderException(ExceptionDescriptor._not_enumerated, null, "value = " + content1);
			else
			    temp1 = MovementPhaseState.cNamedNumbers[idx1];
			this.eventState = temp1;
			present0[0] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("eventState", "MovementPhaseState");
			...
			if (!coder.isNullValue(source)) {
				...
				throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			    if (this.timing == null)
				this.timing = new TimeChangeDetails();
			    this.timing.decodeValue(coder, source);
			    present0[1] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			...
			if (!coder.isNullValue(source)) {
				...
				throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			    if (this.speeds == null)
				this.speeds = new AdvisorySpeedList();
			    this.speeds.decodeValue(coder, source);
			    present0[2] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
	...
	if (!present0[0])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'eventState'");
	if (!present0[1])
	    this.timing = null;
	if (!present0[2])
	    this.speeds = null;
	...
	try {
	    printer.newLine(writer);
	    writer.print("eventState ");
	    printer.print(this.eventState, writer, this.eventState.cConstantNameList);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.timing != null) {
		...
		printer.newLine(writer);
		writer.print("timing ");
		this.timing.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.speeds != null) {
		...
		printer.newLine(writer);
		writer.print("speeds ");
		this.speeds.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
    ...
    public boolean abstractEqualTo(AbstractData that)
    {
	return equalTo((MovementEvent)that);
    }
    ...
    public boolean equalTo(MovementEvent that) {
	if (!this.eventState.equalTo(that.eventState))
	    return false;
	if (this.timing != null) {
	    if (that.timing == null || !this.timing.equalTo(that.timing))
		return false;
	} else if (that.timing != null)
	    return false;
	if (this.speeds != null) {
	    if (that.speeds == null || !this.speeds.equalTo(that.speeds))
		return false;
	} else if (that.speeds != null)
	    return false;
	if (this.regional != null) {
	...
	MovementEvent copy = (MovementEvent)super.clone();
	copy.eventState = eventState.clone();
	if (timing != null) {
	    copy.timing = timing.clone();
	}
	if (speeds != null) {
	    copy.speeds = speeds.clone();
	}
	if (regional != null) {
    ...
    public int hashCode() {
	int hash = 3;
	hash = 41 * hash + (this.eventState != null ? this.eventState.hashCode() : 0);
	hash = 41 * hash + (this.timing != null ? this.timing.hashCode() : 0);
	hash = 41 * hash + (this.speeds != null ? this.speeds.hashCode() : 0);
