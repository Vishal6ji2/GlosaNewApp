...
import com.example.glosanewapp.j2735.dsrc.Duration;
...
/**
 * Define sample code for the Duration ASN.1 type included in the DSRC ASN.1 module.
 * @see Duration
 */

public class DurationSample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private DurationSample() {}

    /**
     * Create Sample Value.
     */
    public static Duration createSampleValue()
    {
	Duration value = new Duration(0);
	return value;
    }
    
    public static void printValue(Duration value, PrintStream s)
    {
	
	s.print(value.longValue());
    }
    
    public static int encodeDecodeAndPrint(Duration value, int run)
    {
	...
	Duration decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (Duration)coder.decode(source, value);
