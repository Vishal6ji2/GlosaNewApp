...
/**
 * Define sample code for the MsgCount ASN.1 type included in the AddGrpB ASN.1 module.
 * @see MsgCount
 */

public class MsgCountSample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private MsgCountSample() {}

    /**
     * Create Sample Value.
     */
    public static MsgCount createSampleValue()
    {
	MsgCount value = new MsgCount(0);
	return value;
    }
    
    public static void printValue(MsgCount value, PrintStream s)
    {
	
	s.print(value.longValue());
    }
    
    public static int encodeDecodeAndPrint(MsgCount value, int run)
    {
	...
	MsgCount decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (MsgCount)coder.decode(source, value);
