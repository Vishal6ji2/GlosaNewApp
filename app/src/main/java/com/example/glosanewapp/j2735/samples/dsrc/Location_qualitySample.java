...
import com.example.glosanewapp.j2735.dsrc.Location_quality;
...
/**
 * Define sample code for the Location_quality ASN.1 type included in the DSRC ASN.1 module.
 * @see Location_quality
 */

public class Location_qualitySample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private Location_qualitySample() {}

    /**
     * Create Sample Value.
     */
    public static Location_quality createSampleValue()
    {
	Location_quality value = Location_quality.loc_qual_bt1m;
	return value;
    }
    
    public static void printValue(Location_quality value, PrintStream s)
    {
	
	s.print(value.longValue());
    }
    
    public static int encodeDecodeAndPrint(Location_quality value, int run)
    {
	...
	Location_quality decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (Location_quality)coder.decode(source, value);
