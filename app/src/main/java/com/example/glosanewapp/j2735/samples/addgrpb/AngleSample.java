...
import com.example.glosanewapp.j2735.addgrpb.Angle;
...
/**
 * Define sample code for the Angle ASN.1 type included in the AddGrpB ASN.1 module.
 * @see Angle
 */

public class AngleSample extends SampleUtil {
     ...
     * Create Sample Value.
     */
    public static Angle createSampleValue()
    {
	Angle value = new Angle(0);
	return value;
    }
    
    public static void printValue(Angle value, PrintStream s)
    {
	
	s.print(value.longValue());
    }
    
    public static int encodeDecodeAndPrint(Angle value, int run)
    {
	...
	try {
	    ...
	    decoded = (Angle)coder.decode(source, value);
