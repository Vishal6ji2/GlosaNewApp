...
import com.example.glosanewapp.j2735.dsrc.MessageBLOB;
...
/**
 * Define sample code for the MessageBLOB ASN.1 type included in the DSRC ASN.1 module.
 * @see MessageBLOB
 */

public class MessageBLOBSample extends SampleUtil {

    /**
     * The default constructor. The class is not instantiable.
     */
    private MessageBLOBSample() {}

    /**
     * Create Sample Value.
     */
    public static MessageBLOB createSampleValue()
    {
	MessageBLOB value = new MessageBLOB(new byte[]
	{
    ...
    public static void printValue(MessageBLOB value, PrintStream s)
    {
	
	s.print(value);
    }
    
    public static int encodeDecodeAndPrint(MessageBLOB value, int run)
    {
	...
	MessageBLOB decoded = null;
	try {
	    System.out.print("\n\tTracing Information from Decoder...\n\n");
	    decoded = (MessageBLOB)coder.decode(source, value);
