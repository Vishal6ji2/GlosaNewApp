...
/**
 * Define the SignalRequest ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */
...
public class SignalRequest extends Sequence {
    ...
    public PriorityRequestType requestType;
    public IntersectionAccessPoint inBoundLane;
    public IntersectionAccessPoint outBoundLane;
	...
	/**
	 * Define the _seq42 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_SignalRequest;
	...
	__inBoundLane("inBoundLane"),
	__outBoundLane("outBoundLane"),
	__regional("regional"),
	_null_("_null_");
	private String tag;
	private static java.util.HashMap<String, __Tag> map =
	...
	sink.writeBit(false);
	sink.writeBit(data.outBoundLane != null);
	...
	try {
	    RequestID item1 = data.requestID;
	    long temp1 = item1.longValue();
	    ...
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("requestID", "RequestID");
	    throw ee;
	}
	// Encode field 'requestType'
	try {
	    PriorityRequestType item1 = data.requestType;
	    int idx1;
	    ...
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("requestType", "PriorityRequestType");
	    throw ee;
	}
	// Encode field 'inBoundLane'
	try {
	    IntersectionAccessPoint item1 = data.inBoundLane;

	    nbits += IntersectionAccessPoint.encodeValue(coder, sink, item1);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("inBoundLane", "IntersectionAccessPoint");
	    throw ee;
	}
	if (data.outBoundLane != null) {
	    // Encode field 'outBoundLane'
	    try {
		IntersectionAccessPoint item1 = data.outBoundLane;

		nbits += IntersectionAccessPoint.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("outBoundLane", "IntersectionAccessPoint");
		throw ee;
	    }
	}
	...
	boolean has_outBoundLane0 = source.readBit();
	...
	try {
		...
		data.requestID = new RequestID();
	    temp1 = coder.decodeConstrainedWholeNumber(source, 0, 255);
	    if (temp1 > 255)
		throw new DecoderException(ExceptionDescriptor._valueRange, null, temp1);
	    data.requestID.setValue(temp1);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("requestID", "RequestID");
	    throw de;
	}
	// Decode field 'requestType'
	try {
	    int idx1;
	    PriorityRequestType temp1;
		...
		temp1 = PriorityRequestType.valueAt(idx1);
	    } else {
		idx1 = 4 + (int)coder.decodeNormallySmallNumber(source);
		if (idx1 < 0)
		    throw new DecoderException(ExceptionDescriptor._not_enumerated, null, "index = " + idx1);
		temp1 = PriorityRequestType.unknownEnumerator();
	    }
	    data.requestType = temp1;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("requestType", "PriorityRequestType");
	    throw de;
	}
	// Decode field 'inBoundLane'
	try {
	    if (data.inBoundLane == null)
		data.inBoundLane = new IntersectionAccessPoint();
	    data.inBoundLane.decodeValue(coder, source, data.inBoundLane);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext("inBoundLane", "IntersectionAccessPoint");
	    throw de;
	}
	if (has_outBoundLane0) {
	    // Decode field 'outBoundLane'
	    try {
		if (data.outBoundLane == null)
		    data.outBoundLane = new IntersectionAccessPoint();
		data.outBoundLane.decodeValue(coder, source, data.outBoundLane);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("outBoundLane", "IntersectionAccessPoint");
		throw de;
	    }
	} else {
	    data.outBoundLane = null;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	// Encode field 'requestID'
	try {
	    RequestID item1 = this.requestID;
	...
	// Encode field 'requestType'
	try {
	    PriorityRequestType item1 = this.requestType;

	    {
		sink.writeSeparator();
		sink.encodeKey("requestType");
		if (item1.isUnknownEnumerator()) {
		    throw new EncoderException(ExceptionDescriptor._relay_error, null, "relay-safe encoding has not been enabled");
		} else 
	...
	// Encode field 'inBoundLane'
	try {
	    IntersectionAccessPoint item1 = this.inBoundLane;

	    {
		sink.writeSeparator();
		sink.encodeKey("inBoundLane");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	...
	// Encode field 'outBoundLane'
	try {
	    IntersectionAccessPoint item1 = this.outBoundLane;

	    if (item1 != null) {
		{
		    sink.writeSeparator();
		    sink.encodeKey("outBoundLane");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "outBoundLane");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("outBoundLane", "IntersectionAccessPoint");
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			this.requestID.setValue(coder.decodeInteger(source));
			present0[1] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("requestID", "RequestID");
			...
			if (present0[2])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			idx1 = coder.resolveName(PriorityRequestType.cConstantNameList, content1);
			if (idx1 < 0 )
			    temp1 = PriorityRequestType.unknownEnumerator();
			else
			    temp1 = PriorityRequestType.cNamedNumbers[idx1];
			this.requestType = temp1;
			present0[2] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("requestType", "PriorityRequestType");
			...
			if (present0[3])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.inBoundLane == null)
			    this.inBoundLane = new IntersectionAccessPoint();
			this.inBoundLane.decodeValue(coder, source);
			present0[3] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("inBoundLane", "IntersectionAccessPoint");
			...
			if (!coder.isNullValue(source)) {
			    if (present0[4])
				throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			    if (this.outBoundLane == null)
				this.outBoundLane = new IntersectionAccessPoint();
			    this.outBoundLane.decodeValue(coder, source);
			    present0[4] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
	...
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'requestID'");
	if (!present0[2])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'requestType'");
	if (!present0[3])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'inBoundLane'");
	if (!present0[4])
	    this.outBoundLane = null;
	if (!present0[5])
	    this.regional = null;
	return this;
    }
	...
	try {
	    ...
	    writer.print(this.requestID.longValue());
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	try {
	    writer.print(',');
	    printer.newLine(writer);
	    writer.print("requestType ");
	    printer.print(this.requestType, writer, this.requestType.cConstantNameList);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	try {
	    writer.print(',');
	    printer.newLine(writer);
	    writer.print("inBoundLane ");
	    this.inBoundLane.printValue(printer, writer);
	} catch (Exception e) {
	    printer.reportError(e, null, writer);
	}
	if (this.outBoundLane != null) {
		...
		writer.print(',');
		printer.newLine(writer);
		writer.print("outBoundLane ");
		this.outBoundLane.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	...
	if (!this.requestID.equalTo(that.requestID))
	    return false;
	if (!this.requestType.equalTo(that.requestType))
	    return false;
	if (!this.inBoundLane.equalTo(that.inBoundLane))
	    return false;
	if (this.outBoundLane != null) {
	    if (that.outBoundLane == null || !this.outBoundLane.equalTo(that.outBoundLane))
		return false;
	} else if (that.outBoundLane != null)
	    return false;
	if (this.regional != null) {
	...
	copy.requestType = requestType.clone();
	copy.inBoundLane = inBoundLane.clone();
	if (outBoundLane != null) {
	    copy.outBoundLane = outBoundLane.clone();
	}
	if (regional != null) {
	...
	hash = 41 * hash + (this.requestType != null ? this.requestType.hashCode() : 0);
	hash = 41 * hash + (this.inBoundLane != null ? this.inBoundLane.hashCode() : 0);
	hash = 41 * hash + (this.outBoundLane != null ? this.outBoundLane.hashCode() : 0);
