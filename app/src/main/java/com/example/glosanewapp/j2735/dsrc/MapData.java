...
/**
 * Define the MapData ASN.1 type included in the DSRC ASN.1 module.
 * @see Sequence
 */

public class MapData extends Sequence implements JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
    public MinuteOfTheYear timeStamp;
    public MsgCount msgIssueRevision;
    public LayerType layerType;
    public LayerID layerID;
    public IntersectionGeometryList intersections;
    public RoadSegmentList roadSegments;
    public DataParameters dataParameters;
    public RestrictionClassList restrictionList;
	...
	/**
	 * Define the _seq7 ASN.1 type included in the DSRC ASN.1 module.
	 * @see Sequence
	 */
	public static class Sequence_ extends Sequence {
		...
		try {
			...
			REG_EXT_ID_AND_TYPE_OSET oset1 = REGION.reg_MapData;
	...
	__roadSegments("roadSegments"),
	__dataParameters("dataParameters"),
	__restrictionList("restrictionList"),
	__regional("regional"),
	_null_("_null_");
	private String tag;
	private static java.util.HashMap<String, __Tag> map =
	...
	sink.writeBit(data.roadSegments != null);
	sink.writeBit(data.dataParameters != null);
	sink.writeBit(data.restrictionList != null);
	...
	if (data.intersections != null) {
	    ...
	    try {
		IntersectionGeometryList item1 = data.intersections;

		nbits += IntersectionGeometryList.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("intersections", "IntersectionGeometryList");
		throw ee;
	    }
	}
	if (data.roadSegments != null) {
	    // Encode field 'roadSegments'
	    try {
		RoadSegmentList item1 = data.roadSegments;

		nbits += RoadSegmentList.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("roadSegments", "RoadSegmentList");
		throw ee;
	    }
	}
	...
	if (data.restrictionList != null) {
	    // Encode field 'restrictionList'
	    try {
		RestrictionClassList item1 = data.restrictionList;

		nbits += RestrictionClassList.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendFieldContext("restrictionList", "RestrictionClassList");
		throw ee;
	    }
	}
	...
	try {
	    ...
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "MapData");
	    throw ee;
	}
    }
	...
	boolean has_roadSegments0 = source.readBit();
	boolean has_dataParameters0 = source.readBit();
	boolean has_restrictionList0 = source.readBit();
	...
	if (has_intersections0) {
	    ...
	    try {
		if (data.intersections == null)
		    data.intersections = new IntersectionGeometryList();
		data.intersections.decodeValue(coder, source, data.intersections);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("intersections", "IntersectionGeometryList");
		throw de;
	    }
	} else {
	    data.intersections = null;
	}
	if (has_roadSegments0) {
	    // Decode field 'roadSegments'
	    try {
		if (data.roadSegments == null)
		    data.roadSegments = new RoadSegmentList();
		data.roadSegments.decodeValue(coder, source, data.roadSegments);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("roadSegments", "RoadSegmentList");
		throw de;
	    }
	} else {
	    data.roadSegments = null;
	}
	...
	if (has_restrictionList0) {
	    // Decode field 'restrictionList'
	    try {
		if (data.restrictionList == null)
		    data.restrictionList = new RestrictionClassList();
		data.restrictionList.decodeValue(coder, source, data.restrictionList);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendFieldContext("restrictionList", "RestrictionClassList");
		throw de;
	    }
	} else {
	    data.restrictionList = null;
	}
	if (has_regional0) {
 ...
 // coder.setUnknownExtensionFound();
	...
	try {
	    ...
	    DecoderException de = DecoderException.wrapException(e);
	    de.appendFieldContext(null, "MapData");
	    throw de;
	}
    }
	...
	// Encode field 'intersections'
	try {
	    IntersectionGeometryList item1 = this.intersections;
	...
	// Encode field 'roadSegments'
	try {
	    RoadSegmentList item1 = this.roadSegments;

	    if (item1 != null) {
		{
		    sink.writeSeparator();
		    sink.encodeKey("roadSegments");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "roadSegments");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("roadSegments", "RoadSegmentList");
	...
	// Encode field 'restrictionList'
	try {
	    RestrictionClassList item1 = this.restrictionList;

	    if (item1 != null) {
		{
		    sink.writeSeparator();
		    sink.encodeKey("restrictionList");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		sink.writeSeparator();
		coder.encodeAbsentComponent(sink, "restrictionList");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("restrictionList", "RestrictionClassList");
	...
	if (coder.hasMoreProperties(source, true))
		...
		switch (t_tag0) {
			...
			if (!coder.isNullValue(source)) {
				...
				this.intersections = new IntersectionGeometryList();
			...
			if (!coder.isNullValue(source)) {
				...
				this.roadSegments = new RoadSegmentList();
			    this.roadSegments.decodeValue(coder, source);
			    present0[5] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			...
			if (!coder.isNullValue(source)) {
				...
				this.restrictionList = new RestrictionClassList();
			    this.restrictionList.decodeValue(coder, source);
			    present0[7] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
	...
	if (!present0[0])
	    this.timeStamp = null;
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, null, "'msgIssueRevision'");
	if (!present0[2])
	    this.layerType = null;
	if (!present0[3])
	    this.layerID = null;
	...
	if (this.layerType != null) {
		...
		printer.print(this.layerType, writer, this.layerType.cConstantNameList);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.layerID != null) {
		...
		writer.print(this.layerID.longValue());
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.intersections != null) {
		...
		this.intersections.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.roadSegments != null) {
		...
		writer.print(',');
		printer.newLine(writer);
		writer.print("roadSegments ");
		this.roadSegments.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.dataParameters != null) {
		...
		this.dataParameters.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	if (this.restrictionList != null) {
		...
		writer.print(',');
		printer.newLine(writer);
		writer.print("restrictionList ");
		this.restrictionList.printValue(printer, writer);
	    } catch (Exception e) {
		printer.reportError(e, null, writer);
	    }
	}
	...
	if (this.timeStamp != null) {
	    ...
	    return false;
	if (!this.msgIssueRevision.equalTo(that.msgIssueRevision))
	    return false;
	if (this.layerType != null) {
	    if (that.layerType == null || !this.layerType.equalTo(that.layerType))
		return false;
	} else if (that.layerType != null)
	    return false;
	if (this.layerID != null) {
	    if (that.layerID == null || !this.layerID.equalTo(that.layerID))
		return false;
	} else if (that.layerID != null)
	    return false;
	if (this.intersections != null) {
	    if (that.intersections == null || !this.intersections.equalTo(that.intersections))
		return false;
	} else if (that.intersections != null)
	    return false;
	if (this.roadSegments != null) {
	    if (that.roadSegments == null || !this.roadSegments.equalTo(that.roadSegments))
		return false;
	} else if (that.roadSegments != null)
	    return false;
	if (this.dataParameters != null) {
	    if (that.dataParameters == null || !this.dataParameters.equalTo(that.dataParameters))
		return false;
	} else if (that.dataParameters != null)
	    return false;
	if (this.restrictionList != null) {
	    if (that.restrictionList == null || !this.restrictionList.equalTo(that.restrictionList))
		return false;
	} else if (that.restrictionList != null)
	    return false;
	if (this.regional != null) {
	...
	if (roadSegments != null) {
	    copy.roadSegments = roadSegments.clone();
	}
	if (dataParameters != null) {
	    copy.dataParameters = dataParameters.clone();
	}
	if (restrictionList != null) {
	    copy.restrictionList = restrictionList.clone();
	}
	if (regional != null) {
	...
	hash = 41 * hash + (this.roadSegments != null ? this.roadSegments.hashCode() : 0);
	hash = 41 * hash + (this.dataParameters != null ? this.dataParameters.hashCode() : 0);
	hash = 41 * hash + (this.restrictionList != null ? this.restrictionList.hashCode() : 0);
