...
/**
 * Define the SignalStatusList ASN.1 type included in the DSRC ASN.1 module.
 * @see SequenceOf
 */
...
public class SignalStatusList extends SequenceOf<SignalStatus> {
     ...
     * @exclude
     */
    public static int encodeValue(PerCoder coder, OutputBitStream sink, SignalStatusList data)
	...
	while (total_len0 > 0) {
	    try {
		SignalStatus item1 = data.elements.get(idx0);

		nbits += SignalStatus.encodeValue(coder, sink, item1);
	    } catch (Exception e) {
		EncoderException ee = EncoderException.wrapException(e);
		ee.appendElementContext(null, "SignalStatus", idx0);
		throw ee;
	    }
     ...
     * @exclude
     */
    public static SignalStatusList decodeValue(PerCoder coder, InputBitStream source, SignalStatusList data)
	...
	if (data.elements != null)
	    ...
	    data.elements = new java.util.ArrayList<SignalStatus>(fragmentLength0);
	while (idx0 > 0) {
	    try {
		SignalStatus item1 = new SignalStatus();

		data.elements.add(item1);
		item1.decodeValue(coder, source, item1);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
		de.appendElementContext(null, "SignalStatus", total_len0);
		throw de;
	    }
	    --idx0; ++total_len0;
	    if (idx0 == 0) {
	...
	if (total_len0 > 0) {
	    while (true) {
		try {
		    SignalStatus item1 = this.elements.get(idx0);
     ...
     * @exclude
     */
    public SignalStatusList decodeValue(JsonCoder coder, JsonReader source)
	...
	if (this.elements != null)
	    ...
	    this.elements = new java.util.ArrayList<SignalStatus>(total_len0);
	...
	if (coder.hasMoreElements(source, true))
		...
		try {
		    ...
		    DecoderException de = DecoderException.wrapException(e);
		    de.appendElementContext(null, "SignalStatus", idx0);
		    throw de;
		}
		++idx0;
    ...
    public void printValue(DataPrinter printer, java.io.PrintWriter writer)
	    throws Exception
    {
	java.util.ArrayList<SignalStatus> temp0 = this.elements;
    ...
    {
	return equalTo((SignalStatusList)that);
    }
    ...
    public boolean equalTo(SignalStatusList that) {
     ...
     * Clone 'this' object.
     */
    public SignalStatusList clone() {
	SignalStatusList copy = (SignalStatusList)super.clone();
	copy.elements = new java.util.ArrayList<SignalStatus>(elements.size());
	for (SignalStatus element : elements) {
